...
Layers supports **23 language models across 5 providers**, all accessible through a unified interface via Vercel AI Gateway. Two Gemini models can also generate images.
...
| **Image generation** | Gemini 2.5 Flash Image | Native multimodal output |
...
Two Gemini models support native image generation through the standard `generateText` API:

| Model | Price | Notes |
|-------|-------|-------|
| Gemini 2.5 Flash Image | See language pricing | Returns images in `result.files[]` |
| Gemini 3 Pro Image | Higher output cost ($120/M tokens) | High-quality generation |
