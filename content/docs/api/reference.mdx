...
```
https://layers.hustletogether.com/api/v1
```
...
```typescript
{
  ...
  // External Cost Input (for Mirror Factory integration)
  mirror_factory?: {
    base_cost_usd: number;  // Base cost calculated by caller
  };
}
...
```typescript
{
  ...
  // Perplexity sources (only for Perplexity models with web search)
  sources?: Source[];

  // AI SDK compatible metadata (for createOpenAI consumers)
...
interface CostBreakdown {
  base_cost_usd: number;        // Base cost in USD (before margin)
  margin_percent: number;       // Margin percentage applied (e.g., 60)
  total_cost_usd: number;       // Total cost after margin
  credits_before_margin: number;// Credits at base cost
  margin_credits: number;       // Additional credits from margin
  validation: "ok" | "warning"; // Cost validation status
  validation_details?: {        // Only present if validation is "warning"
...
// Perplexity source/citation format
interface Source {
  type: "source";
  sourceType: "url";
  id: string;
...
Get structured JSON responses using `json_object` mode:
...
**Note:** Only `json_object` mode is supported. The `json_schema` mode with full schema validation is not yet implemented. If you need structured output, describe your expected format in the prompt and parse the JSON response.

**Supported models for JSON mode:** Anthropic Claude, Google Gemini, Perplexity Sonar
...
Enable real-time web search with citations:
...
// Web search sources/citations (Perplexity models only)
const sources = data.sources || [];
sources.forEach(source => {
  console.log(`${source.title}: ${source.url}`);
});

// Also available in experimental_providerMetadata for AI SDK compatibility
const metadata = data.experimental_providerMetadata?.perplexity;
```
...
### Cost Transparency (v1.5.0+)

Every response includes a full cost breakdown in the `layers` object:

```typescript
const data = await response.json();

console.log(data.layers.cost_breakdown);
// {
//   base_cost_usd: 0.0035,         // Base provider cost
//   margin_percent: 60,             // Layers margin (60%)
//   total_cost_usd: 0.0056,        // Total cost after margin
//   credits_before_margin: 0.35,   // Credits at base rate
//   margin_credits: 0.21,          // Additional margin credits
//   validation: "ok"               // Cost validation status
// }
```

**External Cost Input (Mirror Factory Integration)**
...
const response = await fetch('/api/v1/chat', {
  ...
  body: JSON.stringify({
    ...
    messages: [{ role: 'user', content: 'Hello!' }],
    mirror_factory: {
      base_cost_usd: 0.0035  // Your calculated cost
    }
  }),
});
...
// Layers validates your cost against its own calculation
if (data.layers.cost_breakdown.validation === 'warning') {
  console.log('Cost discrepancy detected:', data.layers.cost_breakdown.validation_details);
  // {
  //   external_base_cost_usd: 0.0035,    // What you sent
  //   calculated_base_cost_usd: 0.0034,  // What Layers calculated
  //   difference_usd: 0.0001,            // Difference
  //   difference_percent: 2.94           // Difference %
  // }
}
```

**Validation Levels:**
- `"ok"` - Costs match within 5% threshold
- `"warning"` - Costs differ by more than 5% (may indicate pricing drift)

**Note:** The `mirror_factory.base_cost_usd` field is optional. If omitted, Layers calculates the base cost from its own model registry.
